...
Texture2D* DataManager::get_interp_layer(int intensity, int prev_intensity) {
    ...
    //string s = "reconstruct" + std::to_string(intensity) + ".pgm";
    //curr_alpha -> writePGM(s.c_str());//wang.
...
//if (l==122) skel->set(x, y, 1);
    ...
    //string s = "skel" + std::to_string(l) + ".pgm";
    //if (l==122) skel->writePGM("122.pgm");
...
Texture2D* DataManager::getWhiteMap() {
    ...
    glBegin(GL_QUADS);
        ...
        // vertex call sets it to that location
        glTexCoord2f(-1.0, -1.0);
        glVertex2f(-dim[0], -dim[1]);//I don't know why it draws a circle but not a quad. So I had to enlarge the range to make sure it's all white.

        glTexCoord2f(-1.0, 1.0);
        glVertex2f(-dim[0], dim[1]*2);

        glTexCoord2f(1.0, 1.0);
        glVertex2f(dim[0]*2, dim[1]*2);

        glTexCoord2f(1.0, -1.0);
        glVertex2f(dim[0]*2, -dim[1]);
