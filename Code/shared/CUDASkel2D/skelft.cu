...
__global__ void kernelSkel(unsigned char* output, bool* fg, short xm, short ym, 
...
{																			//Output:	output: binary thresholded skeleton
	...
	if (tx>xm && ty>ym && tx<xM && ty<yM) 
	{
	  ...
	  Imp = min(Imp,fabsf(length-Imp)) / dt;
	  if (fg[id] && Imp>=threshold) output[Id] = 1;									//By filling only 1-values, we reduce memory access somehow (writing to output[] is expensive)
	} 
...
void skelft2DSkeleton(unsigned char* outputSkel, bool* fg,float length, float threshold,	//Compute thresholded skeleton (into pbaTextures[3]) from resident FT (in pbaTextures[1])
					  short xm,short ym,short xM,short yM)						
{																				//length:     boundary length
    ...
    kernelSkel<<< grid, block >>>((unsigned char*)pbaTextures[3], fg, xm, ym, xM-1, yM-1, pbaTexSize, threshold, length);
